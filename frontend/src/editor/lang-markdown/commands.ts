...
function getContext(node: SyntaxNode, doc: Text) {
  ...
  for (let i = nodes.length - 1; i >= 0; i--) {
    ...
    if (n.name === 'Blockquote' && (match = /^ *>( ?)/.exec(line.text.slice(startPos)))) {
      context.push(new Context(n, startPos, startPos + match[0].length, '', match[1], '>', null));
    } else if (
      n.name === 'ListItem' &&
      ...
      context.push(new Context(n.parent!, startPos, startPos + len, match[1], after, match[2], n));
    } else if (
      n.name === 'ListItem' &&
      ...
      context.push(new Context(n.parent!, startPos, startPos + len, match[1], after, type, n));
    }
  }
  return context;
}
...
/// Patched: never inserts an extra blank line to preserve a non-tight list
/// format. Empty list items always exit one level on Enter.
export const insertNewlineContinueMarkupCommand =
  (): StateCommand =>
  ({ state, dispatch }) => {
    const tree = syntaxTree(state);
    const { doc } = state;
    let dont: { range: ReturnType<typeof EditorSelection.cursor> } | null = null;
    const changes = state.changeByRange((range) => {
      ...
      // Empty line in list — always exit one level. PATCH: upstream had a
      // branch that instead inserted a blank line to make a tight list
      // non-tight; we don't want that.
      ...
      // PATCH: upstream prepends `blankLine(...) + lineBreak` to `insert` when
      // `nonTightList(inner.node, doc)` is true, doubling the newline. Removed
      // so non-tight lists continue tightly.
